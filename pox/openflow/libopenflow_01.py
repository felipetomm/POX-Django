...
class ofp_match (ofp_base):
  #FELIPE TOMM - TCC
  #Essa classe aqui e moda faca. Estudar mais ela
  ...
  def from_packet (cls, packet, in_port = None, spec_frags = False):
    ...
    #FELIPE TOMM - TCC
    #Esta funcao tambem e importante, porem, nao sei se e o que preciso
    #arq = open("/home/mininet/pox/tcc.txt","w")
    #log.debug("Debug TCC - IN_PORT: %s DL_VLAN: %s DL_SRC: %s DL_DST: %s DL_TYPE: %s" %(match.in_port,match.dl_vlan,match.dl_src,match.dl_dst,match.dl_type))
    #log.debug("Debug TCC - NW_SRC: %s NW_DST: %s NW_PROTO: %s NW_TOS: %s" %(match.nw_src,match.nw_dst,match.nw_proto,match.nw_tos))
    #log.debug("Debug TCC - TP_SRC: %s TP_DST: %s" %(match.tp_src,match.tp_dst))
    return match
...
class ofp_flow_mod (ofp_header):
  #FELIPE TOMM - TCC
  #Essa classe trabalha com a modificacao de Flows
  #Depois de modificada, preciso atualizar no BD/Sistema. Ou nao, pois so vou coletar e analisar os conflitos
  _MIN_LENGTH = 72
  def __init__ (self, **kw):
    ofp_header.__init__(self)
    if 'match' in kw:
    ...
    #log.debug("Debug TCC FLOW_MOD - %s " %(self))
    self.show()
  ...
  def pack (self):
    ...
    if po:
      ...
      packed += po.pack()
    #FELIPE TOMM - TCC
    #log.debug("Debug TCC FLOW_MOD PACKED - %s " %(packed))
    return packed
  ...
  def show (self, prefix=''):
    ...
    for obj in self.actions:
      outstr += obj.show(prefix + '  ')
    #FELIPE TOMM - TCC
    #log.debug("Debug TCC FLOW_MOD OUTSTR - %s " %(outstr))
    return outstr
...
class ofp_flow_stats (ofp_stats_body_base):
  #FELIPE TOMM - TCC
  #Aqui posso ganhar uns pontos. Coletar as statistics das flows - Ver tambem a classe anterior
  _MIN_LENGTH = 88
  def __init__ (self, **kw):
    self.table_id = 0
    self.match = ofp_match()
...
class ofp_table_stats (ofp_stats_body_base):
  #FELIPE TOMM - TCC
  #Aqui tambem posso coletar statistics
  def __init__ (self, **kw):
    self.table_id = 0
    self.name = ""
    self.wildcards = 0
...
class ofp_port_stats (ofp_stats_body_base):
  #FELIPE TOMM - TCC
  #Coletar algumas statistics das portas do switch
  def __init__ (self, **kw):
    self.port_no = OFPP_NONE
    self.rx_packets = 0
    self.tx_packets = 0
...
class ofp_flow_removed (ofp_header):
  def __init__ (self, **kw):
    ...
    self.byte_count = 0
    #FELIPE TOMM - TCC
    print ("Debug TCC - LIBOPENFLOW_01 - ofp_flow_removed - Init Flows: %s" %(kw))
    initHelper(self, kw)
    #FELIPE TOMM - TCC
    #Aqui posso implementar para atualizar as flows do banco
    #quando uma flow e removida
  ...
  def show (self, prefix=''):
    ...
    outstr += prefix + 'byte_count: ' + str(self.byte_count) + '\n'
    #FELIPE TOMM - TCC
    print ("Debug TCC - LIBOPENFLOW_01 - ofp_flow_removed - Show Flows: %s" %(outstr))
    return outstr
...
#FELIPE TOMM - TCC
#Cabecalho do pacote. Campos que formam a Flow
ofp_match_data = {

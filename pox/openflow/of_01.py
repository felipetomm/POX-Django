...
def handle_FEATURES_REPLY (con, msg):
  ...
  def finish_connecting (event):
    if event.xid != barrier.xid:
      ...
      #FELIPE TOMM - TCC
      #AQUI EU COLETO O SWITCH
      #dj_switch = ScdComutador.objects.get_or_create(comut_nome=con.dpid)
      log.info("Switch %s %s" %(con.dpid,con.ports))
...
class Connection (EventMixin):
  ...
  def send (self, data):
    ...
    """
    #print "Debug DEF SEND"
    #print data
    if self.disconnected: return

    #TESTE GET FLOW STATS
    """
    if isinstance(data,of.ofp_stats_request):
      print "ENTROU GET FLOW STATS"
      #print data
    #MODULO DE COLETA DAS REGRAS
    if isinstance(data,of.ofp_flow_mod):
      #FELIPE TOMM - TCC
      #AQUI A APLICACAO COLETA AS REGRAS

      if data.idle_timeout is None: pscd_flow_idle_timeout = '0'
      else: pscd_flow_idle_timeout = data.idle_timeout
      if data.hard_timeout is None: pscd_flow_hard_timeout = '0'
      else: pscd_flow_hard_timeout = data.hard_timeout
      #if data.priority is None: pscd_flow_priority = 32000  #ESSE AQUI VAI PEGAR 32768 SE FOR NONE
      #else: pscd_flow_priority = data.priority
      #if data.match._in_port is None: pscd_flow_match_in_port = "*"
      #else: pscd_flow_match_in_port = data.match._in_port
      #if data.actions[0] is None: pscd_flow_actions = "DROP"
      #else: pscd_flow_actions = data.actions[0]
      #if data.match.nw_src is None: pscd_flow_match_nw_src = "*"
      #else: pscd_flow_match_nw_src = data.match.nw_src
      #if data.match.nw_dst is None: pscd_flow_match_nw_dst = "*"
      #else: pscd_flow_match_nw_dst = data.match.nw_dst
      #if data.match.nw_proto is None: pscd_flow_match_nw_proto = "*"
      #else: pscd_flow_match_nw_proto = data.match.nw_proto
      #if data.match.nw_tos is None: pscd_flow_match_nw_tos = "*"
      #else: pscd_flow_match_nw_tos = data.match.nw_tos
      #if data.match._dl_dst is None: pscd_flow_match_dl_dst = "*"
      #else: pscd_flow_match_dl_dst = data.match._dl_dst
      #if data.match._dl_src is None: pscd_flow_match_dl_src = "*"
      #else: pscd_flow_match_dl_src = data.match._dl_src
      #if data.match._dl_type is None: pscd_flow_match_dl_type = "*"
      #else: pscd_flow_match_dl_type = data.match._dl_type
      #if data.match._dl_vlan is None: pscd_flow_match_dl_vlan = "*"
      #else: pscd_flow_match_dl_vlan = data.match._dl_vlan
      #if data.match.tp_dst is None: pscd_flow_match_tp_dst = "*"
      #else: pscd_flow_match_tp_dst = data.match.tp_dst
      #if data.match.tp_src is None: pscd_flow_match_tp_src = "*"
      #else: pscd_flow_match_tp_src = data.match.tp_src

      pscd_flow_table_id = 0
      pscd_flow_priority = data.priority
      pscd_flow_match_in_port = data.match._in_port
      pscd_flow_actions = 'DROP'

      pscd_flow_match_nw_src = data.match.nw_src
      pscd_flow_match_nw_dst = data.match.nw_dst
      pscd_flow_match_nw_proto = data.match.nw_proto
      pscd_flow_match_nw_tos = data.match.nw_tos

      pscd_flow_match_dl_dst = data.match._dl_dst
      pscd_flow_match_dl_src = data.match._dl_src
      pscd_flow_match_dl_type =data.match._dl_type
      pscd_flow_match_dl_vlan =data.match._dl_vlan

      pscd_flow_match_tp_dst = data.match.tp_dst
      pscd_flow_match_tp_src = data.match.tp_src

      dj_flow_table = ScdFlowtable.objects.get_or_create(id_comutador=1)
      dj_flow = ScdFlow(id_flowtable=pscd_flow_table_id,fl_dl_dst=pscd_flow_match_dl_dst,\
                                              fl_dl_src=pscd_flow_match_dl_src,\
                                              fl_dl_type=pscd_flow_match_dl_type,\
                                              fl_dl_vlan=pscd_flow_match_dl_vlan,\
                                              fl_nw_src=pscd_flow_match_nw_src,\
                                              fl_nw_dst=pscd_flow_match_nw_dst,\
                                              fl_nw_tos=pscd_flow_match_nw_tos,\
                                              fl_nw_proto=pscd_flow_match_nw_proto,\
                                              fl_idle_timeout = pscd_flow_idle_timeout,\
                                              fl_hard_timeout = pscd_flow_hard_timeout,\
                                              fl_priority=pscd_flow_priority,\
                                              fl_in_port=pscd_flow_match_in_port,\
                                              fl_actions=pscd_flow_actions,\
                                              fl_tp_dst=pscd_flow_match_tp_dst,\
                                              fl_tp_src=pscd_flow_match_tp_src)
      dj_flow.save()
      """
